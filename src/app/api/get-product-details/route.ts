...
export async function GET(req: NextRequest) {
  try {
    ...
    if (prices.data.length === 0) {
      return NextResponse.json({ error: 'Product does not have any prices assigned' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      priceId: price.id,
      priceFormatted: (price.unit_amount / 100).toFixed(2) + ' ' + price.currency.toUpperCase(),
